...
### Speech-to-Text (`--stt`)
- `core/stt.py`: transcribes voice messages and round video notes through Telegram Premium's server-side speech-to-text. Telethon has no high-level wrapper, so it calls the raw `functions.messages.TranscribeAudioRequest(peer, msg_id)` → `messages.TranscribedAudio(transcription_id, text, pending, …)`. A `pending=True` result normally completes via `UpdateTranscribedAudio`; instead of subscribing, the request is re-issued with `PENDING_BACKOFF_SECONDS` (2/4/6 s) backoff — Telegram returns the same, now-finished transcription — and is given up on if still pending. A `FloodWaitError` up to `FLOOD_WAIT_MAX_SECONDS` (30 s) is waited out; a longer one aborts the whole pass so the export is never stalled by it.
