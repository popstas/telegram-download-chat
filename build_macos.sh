...
# Sign the app with ad-hoc signature (no developer ID required, but GateKeeper won't complain as much)
codesign --force --deep --sign - --timestamp --options=runtime "$SCRIPT_DIR/dist/telegram-download-chat.app"
...
codesign --force --sign - --timestamp "$SCRIPT_DIR/dist/$DMG_NAME"

# Set the DMG to not show the "untrusted app" warning
xattr -c "$SCRIPT_DIR/dist/$DMG_NAME"

# Verify the DMG
hdiutil verify "$SCRIPT_DIR/dist/$DMG_NAME"
