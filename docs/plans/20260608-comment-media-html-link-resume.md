...
- [ ] Add a small opt-in/manual check (or a one-off script under `scripts/`, not committed to the suite) that fetches ~50 messages from discussion group `1619992925` and prints, for forwarded roots, `fwd_from.channel_post`; and for replies, `reply_to.reply_to_top_id` / `reply_to.reply_to_msg_id`. Confirm `root_to_post` + `top_id` mapping reconstructs the same `comment_of` values the per-post path produced for posts 5477/5445.
- [ ] Capture 2–3 representative serialized discussion messages (a forwarded root, a direct reply, a nested reply) as JSON fixtures under `tests/` for the mapping unit tests.
- [ ] ⚠️ If the live structure differs from the assumption, update this plan's mapping design before coding.

### Task B2: Add a pure mapping function (discussion messages → normalized comments)
- [ ] In `core/comments.py`, add `map_discussion_to_comments(downloader, discussion_messages, post_ids, *, limit, min_reactions)` that: builds `root_to_post` from forwarded roots (`fwd_from.channel_post`), skips root/service messages, maps each comment via `reply_to_top_id or reply_to_msg_id`, keeps only comments whose post is in `post_ids` (in-window), normalizes via `_normalize_comment`, then applies per-post `limit` (group-by-post cap) and `min_reactions`. Returns `(comments, raw_media_messages)`.
- [ ] Comments whose root can't be mapped to an in-window post are skipped with a debug log (documented fallback).
- [ ] Write unit tests using the Task B1 fixtures: direct reply maps to its post; nested reply maps via `reply_to_top_id`; a reply to an out-of-window post is dropped; `limit` caps per post; `min_reactions` filters; forwarded roots are not emitted as comments.
- [ ] Run `pytest tests/test_comments.py -k map_discussion` — PASS.
